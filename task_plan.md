...
修正安装脚本仓库地址，增加安装契约；使用实际 Release 资产验证 checksum、版本、帮助和 `deconstruct --help`。已通过临时安装目录验证，补丁版本远端回归待阶段 218。

## 阶段 217：发布后反馈记录与门禁

状态：`in_progress`

新增 `docs/post-release-feedback.md`，记录环境、资产、步骤、预期/实际和严重级别；通过完整测试、vet、race、脚本语法、链接和敏感信息检查。

## 阶段 218：补丁版本发布

状态：`planned`

提交安装链修复并发布 `v0.1.2`，不移动或删除已公开的 `v0.1.1`；核验 CI、Release、Docker 和七项资产。
