...
func commandRegistryInstance() commandRegistry {
	return newCommandRegistry([]slashCommandSpec{
		...
		},
		{
			...
			Group:       "writing",
			Usage:       "/simulate",
			Description: "读取 ./simulate 生成或增量更新拆文方法画像",
			NeedsIdle:   true,
			Run: func(m Model, args []string) (tea.Model, tea.Cmd) {
				m.simSeq++
				...
				if err != nil {
					m.applyEvent(host.Event{
						Time: time.Now(), Category: "ERROR", Summary: "拆文方法画像启动失败：" + err.Error(), Level: "error",
					})
					m.refreshEventViewport()
					return m, nil
				}
		...
		},
		{
			...
			Description: "导入已有拆文方法画像并按语料指纹合并",
			NeedsIdle:   true,
			Run: func(m Model, args []string) (tea.Model, tea.Cmd) {
				m.simSeq++
				...
				if err != nil {
					m.applyEvent(host.Event{
						Time: time.Now(), Category: "ERROR", Summary: "导入拆文方法画像失败：" + err.Error(), Level: "error",
					})
					m.refreshEventViewport()
					return m, nil
				}
