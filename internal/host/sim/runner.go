...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	go func() {
		...
		emit(StageScan, 0, 0, "扫描本地拆文语料...", nil)
		sources, err := scanSources(opts.SourceDir)
		if err != nil {
			emit(StageError, 0, 0, "扫描本地拆文目录失败", err)
			return
		}
		if len(sources) == 0 {
			emit(StageError, 0, 0, "本地拆文目录中没有可分析的 .txt/.md/.markdown 文件", fmt.Errorf("no simulation sources"))
			return
		}
		...
		for i, source := range pending {
			...
			emit(StageAnalyze, i+1, len(pending), fmt.Sprintf("拆解本地语料 %d/%d：%s", i+1, len(pending), source.RelativePath), nil)
		...
		emit(StageMerge, len(pending), len(pending), "合并拆文方法画像...", nil)
		...
		if err := deps.Store.Simulation.Save(profile); err != nil {
			emit(StageError, len(pending), len(pending), "保存拆文方法画像失败", err)
			return
		}
		emit(StageDone, len(pending), len(pending), fmt.Sprintf("拆文方法画像已更新：新增/变更 %d 篇，累计 %d 篇", len(pending), len(profile.Corpus.Sources)), nil)
	}()
	return events, nil
}
	...
	out, err := llmcontract.Execute(ctx, model, llmcontract.Request[T]{
		...
		Hooks: llmcontract.Hooks{
			Resolved: func(res llmcontract.Resolution) {
				slog.Debug("拆文方法画像结构化协议选择", "contract", contract.Name,
			...
			Correction: func(ev llmcontract.Correction) {
				slog.Warn("拆文方法画像输出自愈", "contract", contract.Name, "attempt", ev.Attempt,
