...
4. 这是可机械枚举的默认规则（禁词 / 阈值）？→ `internal/rules/snapshot.go` 的 `SystemDefaults()`；用户自定义规则写进 `.ainovel/rules/*.md`，由归一化快照消费。明确单章目标可进入 `structured.chapter_target_chars`，只限制超过 120% 的正文；范围、全书规模和含糊篇幅仍走 preferences。
