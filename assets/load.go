...
// WithSimulationGuidance 给核心 prompt 追加拆文方法画像指引。导出供 eval 等外部场景做
...
// 否则 baseline 带方法画像后缀、variant 不带，A/B 不等价。file 为 prompt 文件名。
...
const simulationGuidance = `## 拆文方法画像

当 novel_context 的 planning_memory 或 working_memory 中存在 simulation_profile 时，必须把它视为当前作品的抽象方法参考。{{role}} 应读取其中的 style、lexicon、plot_design、hook_design、pacing_density、reader_engagement 和 role_guidance。

使用原则：借鉴结构、节奏、钩子、信息释放和吸引读者的抽象手法；不得模仿具体作者，不得复制原文句子、人物、地名、专有设定、签名短语或固定桥段。若 simulation_profile 与用户显式要求冲突，优先服从用户要求。`
