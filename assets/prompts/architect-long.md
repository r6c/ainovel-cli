...
- **处理过期 fingerprint**：如果工具结果包含 `stale_foundation_fingerprint`，使用结果中的 `current_fingerprint` 重新调用 `audit_foundation`；不要重复提交旧 fingerprint，也不要把该结果当作审查通过。
...
当且仅当 `reference_pack.references.platform_rubric` **存在时**，把它作为规划的**软参考**。用户偏好、故事核心、人物逻辑和后续章节合同优先；不得机械规定冲突数量、钩子数量、爽点密度或固定章数，不得照抄参考文本，也不得为了平台标签破坏长期升级、关系演化或终局方向。
