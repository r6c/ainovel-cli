...
- 注意角色别名，同一人不同称呼不要误判
- 对照 `episodic_memory.knowledge_boundaries` 检查角色是否越权知情：角色不得使用尚未获知的作者真相；有 active 错误信念时，行动与判断应符合误信内容
- 检查认知纠正：角色 `learn` Truth 后不得继续按已纠正错误信念行动，也不能把角色误解写成作者客观事实
- 检查读者揭示节奏：不得提前泄底或重复揭秘；读者已知但角色未知/误信时，角色仍必须遵守自己的认知边界；伏笔 `partial_payoff` 不自动等于完整读者揭示
...
- 只审阅或返工一章时，使用 `scope=chapter`，`chapter` 填该章。
- 用户指定若干章节返工、但任务没有明确给出完整弧的起止章节和弧末时，使用 `scope=global`；issues[].chapters 只填写实际有证据的章节。
- 只有任务明确给出弧的起止章节和弧末章节，且审阅章节就是该弧末时，才使用 `scope=arc`。
- 不要因为章节属于某一弧，就自行把普通多章返工包装成弧级评审；弧级评审必须提交弧末章节，否则工具会拒绝。
- 如果 `save_review` 返回“不是弧末章节”，不要重复提交相同的 `scope=arc` 和章节号：多章返工改用 `scope=global`，单章返工改用 `scope=chapter`。
