...
- 本轮规划阶段不调用 Provider；`v0.1.2` 已完成稳定版发布后安装链验收。
...
### 阶段 AB0：外部更新证据与许可证登记——pending

截至 2026-08-27，已核对五个仓库的默认分支、最新提交/版本和许可证。只保存抽象结论，不复制外部代码、Prompt 或长文档。

### 阶段 AB1：推理内容隔离专项——pending

先用确定性非流式/流式响应测试确认 reasoning_content、think 标签、tool call 和 JSON/正文边界，再决定是否在 Provider/agentcore 转换层做最小修复。

### 阶段 AB2：上游 ainovel-cli 差异审查——pending

针对 LatestCompleted、分层完成补偿、ChapterRecord 候选准备和返工伏笔恢复做行为等价测试；不直接 merge 或 cherry-pick。

### 阶段 AB3：字数口径与完成收口复核——pending

对照 oh-story 的 visible_chars_v1 与当前 domain.WordCount、chapter_target_chars、120% 上限和一次压缩语义；若口径一致，只保留测试/文档结论。

### 阶段 AB4：作者记忆边界设计——pending

评估 UserRules 与跨项目作者记忆的边界；只设计显式记住、确认、回执、冲突和撤回，不直接创建第二套偏好真理。

### 阶段 AB5：细纲照搬检测候选——pending

确认 ChapterContract/大纲是否有稳定可比文本；先做 advisory 证据，不直接硬阻断、不复制外部 JS。

### 阶段 AB6：设定词典候选——pending

确认是否能复用 Knowledge/ReaderKnown；优先诊断首现术语线索，不新增平行认知状态机。

### 阶段 AB7：最终范围与路线收口——pending

全量测试、文档更新、许可证记录和 Go/No-Go；外部候选可暂停且不阻塞稳定版本。
...
- 不复制 GPL-3.0 代码；
- 不复制未明确许可仓库的文本/Prompt；
- 不引入第二个去 AI 味管线；
- 不引入扫榜、Chrome/CDP、数据库或 Web 工作台；
- 不在没有确定性响应证据前修改 Provider 解析；
- 不调用真实 Provider。
