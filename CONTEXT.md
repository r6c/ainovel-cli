...
稳定架构详见 [`docs/architecture.md`](docs/architecture.md)；Context 选择决策矩阵见 [`docs/context-policy-decision-matrix.md`](docs/context-policy-decision-matrix.md)；作者记忆边界见 [`docs/author-memory-boundary.md`](docs/author-memory-boundary.md)；细纲—正文重合边界见 [`docs/outline-copy-boundary.md`](docs/outline-copy-boundary.md)；设定术语边界见 [`docs/setting-term-boundary.md`](docs/setting-term-boundary.md)；发布前自动化与人工验收见 [`docs/release-acceptance.md`](docs/release-acceptance.md)。
...
本规则资产已选择性吸收 `lieflat-less-ai-tone` 的反误报边界，但仍由 ainovel-cli 自己维护：密集顿号只指无功能的空清单；翻译腔只限五类可定位结构；已有具体数据不得被概括表述覆盖；新增算子必须先抽查至少 20 个实际命中实例。上述内容是 Writer/Editor 的白名单语义参考，不是自动改写、AI 来源检测或 Commit 硬门禁。

校准证据位于 `evals/anti-ai-tone/`：16 条自建匿名网文最小对、独立金标、三轮盲评和 Writer 三重复 A/B。外部 `lieflat-less-ai-tone` 仅作为 MIT 许可的设计参考，未安装 Skill、未运行其 Python 脚本、未复制其代码/长文本、未采用其不可复核统计阈值，也不新增第二条去 AI 味流程。
